...
ssize_t readn(int fd, void *ptr, size_t n) {  
   ...
   //restituisce il numero di byte letti con successo
...
int xcrea_socket (char* HOST, int PORT) {
  //crea un socket e si connette a un server
  int fd_skt = 0;
  struct sockaddr_in serv_addr;

  // crea socket
  if ((fd_skt = socket(AF_INET, SOCK_STREAM, 0)) < 0) 
      xtermina("Errore creazione socket", __LINE__, __FILE__);
  // assegna indirizzo
  ...
  if (connect(fd_skt, &serv_addr, sizeof(serv_addr)) < 0) 
      xtermina("Errore apertura connessione", __LINE__, __FILE__);

  return fd_skt;
}

void xinvio_stringa (char* str, int fd_skt) {
  size_t e;
  //invio str
  e = writen(fd_skt, (void*) str, strlen(str));
  if(e!=strlen(str)) xtermina("Errore write", __LINE__, __FILE__);
  //ho mandato str
}

void xinvio_intero (size_t n, int fd_skt) {
  short int tmp;
  size_t e;
  //invio n
  tmp = htons((short int)n);
  e = writen(fd_skt, (void*)&tmp, sizeof(tmp));
  if(e!=sizeof(tmp)) xtermina("Errore write", __LINE__, __FILE__);
}
